...
int task_join (task_t *task){
    ...
    // insere a tarefa atual na fila apontada por queue;
...
void task_suspend (task_t **queue) {
    ...
    // retira a tarefa apontada por task da fila apontada por queue;
...
void task_exit (int exit_code){
    ...
    if(current_task == dispatcher_task){
        ...
        current_task->end_time = time_elapsed;
        for(int i = 0; i < queue_size(current_task->wait_queue); i++){
